...
const schemaProfissional = yup.object({
...
export const FormProfissional = ({btnTitle, onAction, isAdmin}) => {
   const {id} = useParams()
   const {
      register,
      handleSubmit,
      formState: {errors},
      reset
   } = useForm({resolver: yupResolver(schemaProfissional)})
   
   
   useEffect(() => {
      const profissional = profissionaisDb.find(profissional => profissional.id === parseInt(id))
      if(id && profissional){
         reset({
